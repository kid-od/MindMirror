...
- 后端：`backend/`
  ...
  - [document_loader.py](backend/document_loader.py)：PDF/Word/Excel/Markdown 加载与分片。
...
### 3) 文档入库链路
1. 前端上传 PDF/Word/Excel/Markdown 到 `POST /documents/upload`，或通过流式接口批量上传多个文件。
...
- 文档（管理员权限）
  ...
  - `POST /documents/upload`：上传并向量化单个 PDF/Word/Excel/Markdown。
  - `POST /documents/upload/stream`：流式上传并向量化一个或多个 PDF/Word/Excel/Markdown，返回逐文件进度和批量汇总。
