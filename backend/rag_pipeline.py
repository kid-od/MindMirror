...
from config import normalize_base_url, resolve_grade_model
...
GRADE_MODEL = resolve_grade_model(MODEL, os.getenv("GRADE_MODEL"))
...
GRADE_PROMPT = (
    ...
    "Return the result as JSON with the field 'binary_score'."
)
...
def _sanitize_model_response(response: object) -> object:
    ...
    if isinstance(response, BaseModel):
        updates = {}
        if "parsed" in response.__class__.model_fields and isinstance(getattr(response, "parsed", None), BaseModel):
            updates["parsed"] = None
        if updates:
            sanitized = response.model_copy(update=updates)

    parsed_value = getattr(sanitized, "parsed", None)
    if isinstance(parsed_value, BaseModel):
        try:
            setattr(sanitized, "parsed", None)
        except Exception:
            pass

    for field_name in ("additional_kwargs", "response_metadata"):
        payload = getattr(sanitized, field_name, None)
        if isinstance(payload, dict) and isinstance(payload.get("parsed"), BaseModel):
            payload = dict(payload)
            payload["parsed"] = None
            try:
                setattr(sanitized, field_name, payload)
            except Exception:
                original_payload = getattr(sanitized, field_name, None)
                if isinstance(original_payload, dict):
                    original_payload["parsed"] = None
...
def _extract_binary_score(response: object) -> str:
    if hasattr(response, "binary_score"):
        score = str(getattr(response, "binary_score", "") or "").strip().lower()
        if score in {"yes", "no"}:
            return score

    text = _response_text(response)
    payload = _parse_json_object(text)
    score = str(payload.get("binary_score", "") or "").strip().lower()
    if score in {"yes", "no"}:
        return score

    lowered = text.lower()
    if re.search(r'\bbinary_score\b[\s:=\'"]*yes\b', lowered) or re.search(r"\byes\b", lowered):
        return "yes"
    if re.search(r'\bbinary_score\b[\s:=\'"]*no\b', lowered) or re.search(r"\bno\b", lowered):
        return "no"
    return "no"


def _extract_rewrite_strategy(response: object) -> str:
    allowed = {"step_back", "hyde", "complex"}

    if hasattr(response, "strategy"):
        strategy = str(getattr(response, "strategy", "") or "").strip().lower()
    ...
    lowered = text.lower()
    for candidate in ("complex", "step_back", "hyde"):
...
def grade_documents_node(state: RAGState) -> RAGState:
    ...
    response = _sanitize_model_response(grader.invoke([{"role": "user", "content": prompt}]))
...
def rewrite_question_node(state: RAGState) -> RAGState:
    ...
    if router:
        prompt = (
            ...
            f"用户问题：{question}\n"
            "请以 JSON 形式返回结果，字段名必须为 strategy。"
        )
        try:
            decision = _sanitize_model_response(router.invoke([{"role": "user", "content": prompt}]))
            strategy = _extract_rewrite_strategy(decision)
        except Exception:
            strategy = "step_back"
